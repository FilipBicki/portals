...
# Ensure stability
execute if block ^-0.5 ^-2 ^ minecraft:quartz_block if block ^-1.5 ^-2 ^ minecraft:quartz_block if block ^0.5 ^-2 ^ minecraft:quartz_block if block ^1.5 ^-2 ^ minecraft:quartz_block if block ^1.5 ^-1 ^ minecraft:quartz_block if block ^-1.5 ^-1 ^ minecraft:quartz_block if block ^1.5 ^ ^ minecraft:quartz_block if block ^-1.5 ^ ^ minecraft:quartz_block if block ^1.5 ^1 ^ minecraft:quartz_block if block ^-1.5 ^1 ^ minecraft:quartz_block if block ^1.5 ^2 ^ minecraft:quartz_block if block ^-1.5 ^2 ^ minecraft:quartz_block if block ^0.5 ^2 ^ minecraft:quartz_block if block ^-0.5 ^2 ^ minecraft:quartz_block run tag @s add stable
...
#### Teleport ####
# Overworld to Custom
execute as @e[type=#tf_portals:p1_can_tp,tag=!p1_teleported] at @s if predicate tf_portals:in_overworld if entity @e[tag=p1_extra,distance=..0.8] at @e[tag=portal1,distance=0..2] rotated as @e[tag=portal1,distance=0..2] in connecticut:hartford run function tf_portals:portal1/teleport
# Custom to Overworld
execute as @e[type=#tf_portals:p1_can_tp,tag=!p1_teleported] at @s if predicate tf_portals:in_hartford if entity @e[tag=p1_extra,distance=..0.8] at @e[tag=portal1,distance=0..2] rotated as @e[tag=portal1,distance=0..2] in minecraft:overworld run function tf_portals:portal1/teleport
